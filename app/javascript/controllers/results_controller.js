...
function paintRange(input){const percent=Number(input.value)/Number(input.max)*100;input.style.background='linear-gradient(to right,var(--burgundy) '+percent+'%,var(--hairline) '+percent+'%)';}
...
// Announce the settled figure once per adjustment. Live-updating #total on every
// input frame read the whole drag aloud.
const announce=()=>{root.querySelector('#status').textContent='Annual value now '+root.querySelector('#total').textContent+'.';};
spend.addEventListener('change',announce);
root.querySelector('#category-sliders').addEventListener('change',announce);
renderCards();
  }
}
