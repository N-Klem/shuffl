...
export default class extends Controller {
  ...
  connect() {
    ...
    function refresh(){const guideOpen=stage.querySelector('[data-wallet-guide]')?.open||false;stage.innerHTML=render();const guide=stage.querySelector('[data-wallet-guide]');if(guide)guide.open=guideOpen}
...
// Where the wallet earns extra, by everyday category, in each card's own unit
// (cashback and points side by side, never ranked). A gap hands the question to
// chip, whose save button brings the card straight back into this map.
function coverageMarkup(){
  if(!cards.length||!data.coverage?.length)return '';
  const rate=u=>escape(u.rate)+({cashback_percent:'% cashback',points_per_USD:Number(u.rate)===1?' point per $1':' points per $1',miles_per_USD:Number(u.rate)===1?' mile per $1':' miles per $1'}[u.unit]||'');
  const gaps=data.coverage.filter(row=>!row.cards.length).length;
  const intro=gaps===0?'Every category has a card earning extra on it.':gaps===data.coverage.length?'None of your cards earns extra on these categories yet. Ask chip for one that does.':`${gaps===1?'One category has':gaps+' categories have'} no card earning extra. Ask chip to fill a gap.`;
  // "planned" is only worth marking per card when the wallet mixes owned and planned.
  const allPlanned=!cards.some(c=>c.owned), mixed=!allPlanned&&cards.some(c=>!c.owned);
  const eyebrow=`YOUR ${cards.length===1?'':cards.length+' '}${allPlanned?'PLANNED ':''}${cards.length===1?'CARD':'CARDS'}`;
  return `<section class="coverage" aria-labelledby="coverage-heading"><div class="row"><div><span class="eyebrow">${eyebrow}</span><h2 id="coverage-heading">Category coverage</h2></div></div><p class="timeline-intro">${intro}</p>
  <ul class="coverage-map">${data.coverage.map(row=>`<li class="coverage-cell${row.cards.length?'':' is-gap'}"><span class="coverage-category">${escape(row.category)}</span>${row.cards.length?row.cards.slice(0,2).map(c=>`<span class="coverage-card"><strong>${escape(c.name)}${mixed&&c.planned?' <span class="coverage-note">· planned</span>':''}</strong><span class="reward">${rate(c)}</span></span>`).join('')+(row.cards.length>2?`<span class="muted">+${row.cards.length-2} more</span>`:''):`<span class="coverage-gap">Nothing earns extra here.</span><button class="text" type="button" data-assistant-ask="Which cards earn extra on ${escape(row.category.toLowerCase())}?">Ask chip</button>`}</li>`).join('')}</ul></section>`
}
