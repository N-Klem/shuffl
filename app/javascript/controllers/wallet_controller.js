...
export default class extends Controller {
  ...
  connect() {
    ...
    const money=n=>{const v=Math.round(Number(n)||0);return (v<0?'−$':'$')+Math.abs(v).toLocaleString()};
...
function card(c,button=false,i=0){const tag=button?'button':'div';return `<${tag} class="credit" style="--metal:${c.metal};--fg:${c.fg};--i:${i}" ${button?`type="button" data-select-card="${c.id}" aria-label="Select ${c.name}" aria-pressed="${selected===c.id}"`:''}>${wordmark}<span class="chip"></span><span class="name">${c.name}</span></${tag}>`}
...
function render(){const c=cards.find(x=>x.id===selected),owned=cards.filter(x=>x.owned);return `<main class="${view}"><div class="heading"><div><span class="eyebrow">Your cards. Working together.</span><h1>my <span class="keyword-emphasis">wallet</span></h1></div><div class="summary"><div><span class="muted">Estimated annual rewards · owned</span><span class="number reward">${money(owned.reduce((a,c)=>a+c.reward,0))}</span></div><div><span class="muted">Annual fees · owned</span><span class="number fee">${money(owned.reduce((a,c)=>a+c.fee,0))}</span></div></div></div>
${paymentTimeline()}
<div class="row collection-heading"><h2>Your collection <span class="muted">${cards.length} cards</span></h2><div class="actions"><button class="btn-wallet" data-act="find">Find more cards</button><button class="primary" data-act="plan">Plan my spending</button></div></div>
...
${plannedTimeline()}
<p class="demo">Estimates use Shuffl’s illustrative catalogue and your spending plan. Each category counts once on the best-rate owned card. Points valued at 1¢; excludes bonuses, interest and caps. Tracking is manual.</p></main>`}
function details(c){
  const days=c.deadline?Math.ceil((new Date(c.deadline+'T12:00:00')-new Date(data.today+'T12:00:00'))/86400000):null;
  const active=c.owned?tabName:'Overview';
  ...
  ${c.owned?`<div class="tabs" role="tablist" aria-label="Card details">${['Overview','Payments','Bonus'].map(t=>`<button type="button" role="tab" id="wallet-tab-${t}" aria-controls="wallet-detail-panel" aria-selected="${active===t}" tabindex="${active===t?'0':'-1'}" data-tab="${t}" class="${active===t?'active':''}">${t}</button>`).join('')}</div>`:''}
  <div class="details-body" id="wallet-detail-panel" ${c.owned?`role="tabpanel" aria-labelledby="wallet-tab-${active}" tabindex="0"`:''}>
  ${active==='Overview'?`<p>${c.description}</p><a class="text" href="${c.url}">Explore this card ↗</a><div class="section stats"><div><span class="muted">Annual rewards${c.owned?'':' · standalone estimate'}</span><strong class="reward">${money(c.reward)}</strong></div><div><span class="muted">Annual fee</span><strong class="fee">${money(c.fee)}</strong></div><div><span class="muted">After annual fee</span><strong class="${c.reward-c.fee>=0?'reward':'fee'}">${money(c.reward-c.fee)}</strong></div></div>`:''}
  ...
  ${active==='Bonus'?`<div class="section"><div class="row"><h3>${c.bonus}</h3><span class="muted bonus-deadline">${days==null?'No deadline':days<0?'Deadline passed':days+' days left'}</span></div><p>${c.goal?`${money(Math.max(0,c.goal-c.spent))} remaining of ${money(c.goal)}.`:'Add the spending requirement and deadline from your offer.'}</p><div class="progress"><span style="width:${c.goal?Math.min(100,c.spent/c.goal*100):0}%"></span></div><div class="row"><span class="muted">${money(c.spent)} of ${c.goal?money(c.goal):'—'}</span><button class="btn-wallet" data-act="total">${c.deadline?'Edit bonus tracking':'Set deadline'}</button></div></div>`:''}
    ...
    const onKeydown=e=>{const tab=e.target.closest('[data-tab]');if(!tab||!['ArrowLeft','ArrowRight','Home','End'].includes(e.key))return;e.preventDefault();const names=['Overview','Payments','Bonus'],i=names.indexOf(tabName);tabName=e.key==='Home'?names[0]:e.key==='End'?names[2]:names[(i+(e.key==='ArrowRight'?1:2))%3];refresh();stage.querySelector(`[data-tab="${tabName}"]`)?.focus()};
    accept(this.payloadValue);refresh();stage.addEventListener('click',onClick);stage.addEventListener('keydown',onKeydown);
    this.cleanup=()=>{stage.removeEventListener('click',onClick);stage.removeEventListener('keydown',onKeydown);clearTimeout(toastTimer);dialog.close()};
  }
